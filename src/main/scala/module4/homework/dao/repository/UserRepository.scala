...
object UserRepository{
    ...
    class ServiceImpl extends Service{
        ...
        def insertRole(role: Role): Result[Role] = dc.run(
        ...
        def listUsersWithRole(roleCode: RoleCode): Result[List[User]] = dc.run(
          for {
            userToRole <- userToRoleSchema if lift(roleCode.code) == userToRole.roleId
            user <- userSchema.join(userToRole.userId == _.id)
          } yield user
        )
